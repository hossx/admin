...
class EthTransfer(host: String, port: Int) extends Actor with ActorLogging {

  private case class FetchTransferItem()

  private val mongoUri = MongoURI(s"mongodb://${host}:${port}/eth")
  private val txCollection = MongoConnection(mongoUri)(mongoUri.database.get)("txs")
  ...
  private def processPendingRequest() {
    for (tx <- txCollection.find(MongoDBObject("cps" -> "PENDING", "ty" -> "DEPOSIT"))) {
      try {
        val id = tx.get("_id").asInstanceOf[String]
        val uid = tx.get("c").asInstanceOf[String].replaceAll("""\s+""", "").replaceAll("\"", "").toLong
        val amount = tx.get("a").asInstanceOf[Integer].toDouble
